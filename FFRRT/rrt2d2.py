...
def distance(nodeA, nodeB, weights = weights, useMap = False, newDist = True, distanceScaler = 0.008):
...
def getPosesFromFormation(node):
    ...
    return locs
def getPosesFromFormation2(state):
    global initFormation
    xc = state[0]
    yc = state[1]
    theta = state[2]
...
def isColliding3(state, obstacleMap):
    # x, y = int(node.state[0]), int(node.state[1])
    locs = getPosesFromFormation2(state)
    for loc in locs:
        try:
            if obstacleMap[loc[1], loc[0]] == 255 :
                return True
...
def RRT(dic):
    ...
    for its in tqdm(range(numNodes)):
        ...
        while True:
            ...
            if dic['newSampling'] and sampledImage[loc[1], loc[0]] != 0:
                continue
            # if not isColliding3(loc,obstacleMap):
            ...
            if not isColliding(extendedNode, obstacleMap, steps = edgeCollStep):
                break
        # while ends here
        ...
        for node in neighbourNodes:
            ...
            distb = distance(node, extendedNode, newDist = newDist) + extendedNode.cost
            if  distb < node.cost:
